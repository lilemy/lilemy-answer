...
import com.lilemy.lilemyanswer.model.dto.scoringresult.ScoringResultAddRequest;
import com.lilemy.lilemyanswer.model.dto.scoringresult.ScoringResultEditRequest;
import com.lilemy.lilemyanswer.model.dto.scoringresult.ScoringResultQueryRequest;
import com.lilemy.lilemyanswer.model.dto.scoringresult.ScoringResultUpdateRequest;
...
public class ScoringResultController {
    ...
    public BaseResponse<Long> addScoringResult(@RequestBody ScoringResultAddRequest scoringResultAddRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(scoringResultAddRequest == null, ResultCode.PARAMS_ERROR);
        // 在此处将实体类和 DTO 进行转换
    ...
    public BaseResponse<Boolean> deleteScoringResult(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        scoringResultService.isOwnScoringResult(id, request);
        // 操作数据库
    ...
    @Operation(summary = "更新评分结果（仅管理员可用）")
    @PostMapping("/update")
    ...
    @Operation(summary = "根据 id 获取评分结果（封装类）")
    @GetMapping("/get/vo")
    public BaseResponse<ScoringResultVO> getScoringResultVOById(long id, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(scoringResult == null, ResultCode.NOT_FOUND_ERROR);
        // 获取封装类
        return ResultUtils.success(scoringResultService.getScoringResultVO(scoringResult, request));
    }

    @Operation(summary = "分页获取评分结果列表（仅管理员可用）")
    ...
    @Operation(summary = "分页获取评分结果列表（封装类）")
    ...
    public BaseResponse<Page<ScoringResultVO>> listMyScoringResultVOByPage(@RequestBody ScoringResultQueryRequest scoringResultQueryRequest,
                                                                           HttpServletRequest request) {
        ThrowUtils.throwIf(scoringResultQueryRequest == null, ResultCode.PARAMS_ERROR);
        // 补充查询条件，只查询当前登录用户的数据
        User loginUser = userService.getLoginUser(request);
        scoringResultQueryRequest.setUserId(loginUser.getId());
        return listScoringResultVOByPage(scoringResultQueryRequest, request);
    }

    @Operation(summary = "编辑评分结果（给用户使用）")
    @PostMapping("/edit")
